p.p1 {margin: 0.0px 0.0px 0.0px 0.0px; font: 13.0px 'Helvetica Neue'} p.p2 {margin: 0.0px 0.0px 2.0px 0.0px; font: 16.0px 'Helvetica Neue'} li.li1 {margin: 0.0px 0.0px 0.0px 0.0px; font: 13.0px 'Helvetica Neue'} span.s1 {text-decoration: underline} span.s2 {font: 12.0px 'Helvetica Neue'} ol.ol1 {list-style-type: decimal} ul.ul1 {list-style-type: disc}

Instructions pour l'Agent
Ce fichier est dupliqué dans [CLAUDE.md](http://claude.md/), [AGENTS.md](http://agents.md/) et [GEMINI.md](http://gemini.md/) afin que les mêmes instructions se chargent dans n'importe quel environnement IA.
...
Vous êtes le liant entre l'intention et l'exécution. Par exemple, vous n'essayez pas de scraper des sites web vous-même — vous lisez directives/scrape_website.md, déterminez les entrées/sorties, puis exécutez execution/scrape_single_site.py
...
Tester l'outil, s'assurer qu'il fonctionne
Mettre à jour la directive pour inclure le nouveau flux
Le système est désormais plus robuste
...
.tmp/ - Tous les fichiers intermédiaires (dossiers, données scrapées, exports temporaires). Jamais commités, toujours régénérés.
...
credentials.json, token.json - Identifiants OAuth Google (fichiers requis, dans .gitignore)
